...
def get_jira_connection():
    ...
    user = os.getenv('JIRA_API_USER') or (_ for _ in ()).throw(EnvironmentError("JIRA_API_USER environment variable is not set."))
    token = os.getenv('JIRA_API_TOKEN') or (_ for _ in ()).throw(EnvironmentError("JIRA_API_TOKEN environment variable is not set."))
...
def close_issue(jira_local, issue_key, build_hash):
    ...
    try:
        ...
        close_transition = next(
            (t for t in transitions if t['name'] == 'Closed'), None
        )
        if close_transition:
...
def create_investigation_task_for(jira_local, summary, description, component, environment):
    issue_dict = {
        'project': {'key': 'LPD'},
        "summary": summary,
